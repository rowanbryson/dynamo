...
def average_eps_tri(eps, triaxiality, time, time_at_fracture):
    ...
    avg_eps = np.mean(eps)
    avg_triaxiality = np.mean(triaxiality)
    ...
    eps_at_fracture = eps[idx+1]
    triaxiality_before_fracture = triaxiality[:idx+1]
    triaxiality_at_fracture = triaxiality[idx+1]

    triaxiality_avg_before_fracture = np.nanmean(triaxiality_before_fracture, axis=0)
    # mean and ignore nan values

    # print(triaxiality_avg_before_fracture)

    # plt.figure()
    # plt.plot(triaxiality_before_fracture, eps[:idx+1], 'x', label='Compression Testing')
    # plt.xlabel('Stress Triaxiality')
    # plt.ylabel('Effective Plastic Strain')
